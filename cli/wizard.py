...
WIZARD_CONFIG_DIR = REPO_ROOT / ".wizard"
...
def get_stored_account_id() -> str | None:
    """Get previously stored account ID from wizard config."""
    config_file = WIZARD_CONFIG_DIR / "account_id"
...
def store_account_id(account_id: str) -> None:
    """Store account ID in wizard config for future use."""
    WIZARD_CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    config_file = WIZARD_CONFIG_DIR / "account_id"
...
def ensure_terraform_vars(env: str) -> None:
    ...
    with tfvars_file.open("w") as f:
        f.write("# Auto-generated by wizard CLI\n")
