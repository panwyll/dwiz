...
def get_stored_account_id() -> str | None:
    """Get previously stored account ID from dwiz config."""
    config_file = DWIZ_CONFIG_DIR / "account_id"
...
def store_account_id(account_id: str) -> None:
    """Store account ID in dwiz config for future use."""
    DWIZ_CONFIG_DIR.mkdir(parents=True, exist_ok=True)
    config_file = DWIZ_CONFIG_DIR / "account_id"
...
def get_or_prompt_account_id() -> str:
    ...
    Returns:
        Account ID string
    """
    # First try to get from current AWS credentials
    account_id = get_aws_account_id()
    if account_id:
        print(f"✓ Detected AWS Account ID from credentials: {account_id}")
        # Store it for future use
        store_account_id(account_id)
        return account_id
    
    # Next, try to get from previously stored value
    stored_account_id = get_stored_account_id()
    if stored_account_id:
        print(f"Using previously configured Account ID: {stored_account_id}")
        response = input(f"Continue with account ID {stored_account_id}? [Y/n]: ").strip().lower()
        if response in ("", "y", "yes"):
            return stored_account_id
    
    # Finally, prompt user to enter it
    print()
    print("=" * 80)
    print("AWS Account ID Required")
    print("=" * 80)
    print()
    print("Terraform requires your AWS Account ID to configure OIDC roles.")
    print()
    print("To find your AWS Account ID:")
    print("  1. Run: aws sts get-caller-identity --query Account --output text")
    print("  2. Or check the AWS Console at:")
    print("     https://console.aws.amazon.com/billing/home#/account")
    print()
    
    while True:
        account_id = input("Enter your AWS Account ID (12 digits): ").strip()
        # Validate it's 12 digits
        if account_id.isdigit() and len(account_id) == 12:
            store_account_id(account_id)
            print(f"✓ Account ID {account_id} saved for future use")
            return account_id
        else:
            print("Error: Account ID must be exactly 12 digits. Please try again.")


def ensure_terraform_vars(env: str) -> None:
    """Ensure terraform.tfvars exists for the given environment with required variables.
    ...
    # Update or set account_id
    existing_vars["account_id"] = account_id
    
    # Set OIDC provider ARN if not already set
    if "oidc_provider_arn" not in existing_vars:
        existing_vars["oidc_provider_arn"] = oidc_provider_arn
    
    # Set repo if not already set
    if "repo" not in existing_vars:
        existing_vars["repo"] = repo
    
    # Write tfvars file
    with tfvars_file.open("w") as f:
        f.write("# Auto-generated by dwiz CLI\n")
        f.write("# You can edit these values as needed\n\n")
...
def validate_resource_names(bucket: str, table: str) -> None:
    ...
    if bucket_has_placeholder or table_has_placeholder:
        ...
        if account_id:
            ...
            error_parts.append(
                f"  dwiz bootstrap --bucket {suggested_bucket} --table {suggested_table}"
            )
        else:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="dwiz")
