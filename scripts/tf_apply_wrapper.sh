...
if [ $TERRAFORM_EXIT_CODE -eq 0 ]; then
  ...
  echo "      terraform -chdir=terraform/envs/${ENVIRONMENT} import module.s3_lake.aws_s3_bucket.raw <bucket-name>"
  echo ""
  echo "      # IAM roles:"
  echo "      terraform -chdir=terraform/envs/${ENVIRONMENT} import module.iam_${ENVIRONMENT}.aws_iam_role.github <role-name>"
  echo ""
