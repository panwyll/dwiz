...
This document describes how to securely manage secrets (API keys, database credentials, etc.) in the Data Platform Dwiz.
...
Examples:
- `dwiz-dev/api-keys` - API keys for external services (dev)
- `dwiz-dev/database` - Database credentials (dev)
- `dwiz-dev/streaming` - Streaming service credentials (dev)
- `dwiz-prod/api-keys` - API keys for external services (prod)
...
api_key = get_secret_value("dwiz-dev/api-keys", "github_token")

# Get entire secret as dictionary
api_keys = get_secret("dwiz-dev/api-keys")
...
def process_data():
    ...
    db_host = get_secret_value("dwiz-dev/database", "host")
    db_user = get_secret_value("dwiz-dev/database", "username")
    db_pass = get_secret_value("dwiz-dev/database", "password")
...
def get_env_secret(secret_category: str, key: str) -> str:
    ...
    secret_name = f"dwiz-{env}/{secret_category}"
...
# After
from libs.python_common.secrets import get_secret_value
api_key = get_secret_value("dwiz-dev/api-keys", "external_api")  # Good!
```
